...
function keySentence(text) {
    ...
    let sentsWCap = breakSentace(text, true);
    sents.pop();
    let i = 0;

    //Index sentences in document
    sents.forEach(function (sentencz, index) {
    ...
    let justSents = document.slice(1, 20).map(e=>sentsWCap[e.index]);
    // return index of where the text is
    return [sentsWCap[0], ...justSents];
}
